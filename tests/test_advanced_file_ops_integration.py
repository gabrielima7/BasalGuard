...
from basalguard.tools.advanced_file_ops import search_in_file, read_file_paged
...
def test_file_ops_integration(firewall: BasalGuardCore, workspace: Path):
    ...
    # Test search_in_file
    result = firewall.validate_intent("search_in_file", {"path": "test.txt", "pattern": "Secret"})
    assert result["status"] == "success"
    assert len(result["matches"]) == 1
    assert "Secret: 12345" in result["matches"][0]

    # Test read_file_paged
    result = firewall.validate_intent("read_file_paged", {"path": "test.txt", "offset": 0, "limit": 5})
    assert result["status"] == "success"
    assert result["content"] == "Hello"

    # Test read_file_paged offset
    result = firewall.validate_intent("read_file_paged", {"path": "test.txt", "offset": 6, "limit": 5})
    assert result["status"] == "success"
    assert result["content"] == "World"

    # Test large file read via paged (simulated)
    result = firewall.validate_intent("read_file_paged", {"path": "test.txt", "offset": 0, "limit": 1000})
    assert result["status"] == "success"
    assert len(result["content"]) == len(test_file.read_text())

def test_search_in_file_blocked(firewall: BasalGuardCore):
    # Test path traversal
    result = firewall.validate_intent("search_in_file", {"path": "../../etc/passwd", "pattern": "root"})
    assert result["status"] == "blocked"
    assert "path_traversal" in str(result.get("reason", ""))

def test_read_file_paged_blocked(firewall: BasalGuardCore):
    # Test path traversal
    result = firewall.validate_intent("read_file_paged", {"path": "../../etc/passwd", "offset": 0})
    assert result["status"] == "blocked"
    assert "path_traversal" in str(result.get("reason", ""))
